  ...
  request(requestUrl, function (error, response, body) {
    if (error != null) {
      ...
      if (response.headers['content-type'].indexOf('application/json') > -1 && body != '') {
        ...
        scan(schemaObj);
        var responseName = response.statusCode + "-response";
        var paramRef = {"$ref": "#/definitions/" + responseName};
        swaggerSpec.paths[apiPath][pathMethod]["responses"][response.statusCode].schema = paramRef;
        swaggerSpec.definitions[responseName] = schemaObj;
      }
  ...
  if (urlObj.query != null && urlObj.query.split("&").length > 0) {
    ...
    async.eachSeries(queryParams, function iterator(queryparam, qcallback) {
      ...
      apiq.queryParamQ(keyValue[0], keyValue[1], function (answers) {
        generateParameterEntry(swaggerSpec, apiPath, pathMethod, answers);
        qcallback(null, true);
      });
...
var generateParameterEntry = function (swaggerSpec, apiPath, pathMethod, answers) {
  var paramRef = {};
  paramRef["$ref"] = "#/parameters/" + answers.name;
  swaggerSpec.paths[apiPath][pathMethod]["parameters"].push(paramRef);
  swaggerSpec.parameters[answers.name] = answers;
  console.log(JSON.stringify(swaggerSpec, null, 2));
};